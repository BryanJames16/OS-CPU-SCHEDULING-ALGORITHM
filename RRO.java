...
RRO 
{
	...
	protected 
	RRO (double grandData [][], int jobNumber [], int noOfJobs, double quantum, double overhead) 
	{
	...
	compute () 
	{
		...
		// Core
		for (jobDone = 0; jobDone < noOfJobs; ) {
			for (int count = 0; count < noOfJobs; count++) {
			...
			if (burstCopy [lowestJobIndex] - quantum <= 0) {
				...
				// if job will not die decrementing by the quantum
			...
			if (jobDone != noOfJobs) {
				timer += overhead;
				orderOfJobs [orderCount] = 100;
				orderOfBursts [orderCount] = overhead;
				flagTime [counter2] = timer;
				...
				for (int count = 0; count < stackCount; count++) {
					if (arrivalCopy [lowestJobIndex] == stackArrival [count]) {
						try {
							stack [count][widthCount [count]] = 100;
							isAlive [stackCount][widthCount [stackCount]] = true;
							widthCount [count]++;
							isStacked = true;
				...
				if (!isStacked) {
					stack [stackCount][widthCount [stackCount]] = 100;
					isAlive [stackCount][widthCount [stackCount]] = true;
					stackArrival [stackCount] = arrivalCopy [lowestJobIndex];
					widthCount [stackCount]++;
				...
				System.out.println ("    \t|    " + String.format (" O    |", jobNumber [lowestJobIndex]));
				System.out.println ("    \t|   " + String.format ("%.2f   |", overhead));
				System.out.println (String.format ("%.2f", timer) + "\t|__________|");
		...
		for (int count = 0; count < stackCount; count++) {
			for (int count2 = 0; count2 < widthCount[stackCount]; count2++) {
				if (isAlive [count][count2]) {
					System.out.print (stack [count][count2] + " ");
				} else {
					System.out.print ("(" + stack [count][count2] + ") ");
				}
			}
	...
	launchFrame () 
	{
		RRODrawGantt rrodg = new RRODrawGantt (grandData, jobNumber, ttAve, wtAve, flagTime, orderOfBursts, orderOfJobs, orderCount, noOfJobs, idle, stack, isAlive, stackArrival, widthCount, stackCount, widest);
		rrodg.launchFrame ();
	}
}