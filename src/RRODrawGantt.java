...
public class 
RRODrawGantt implements ActionListener
{
	...
	*/
	/* RR */
	protected 
	RRODrawGantt (double grandData [][], int jobNumber [], double ttAve, double wtAve, double flagTime [], double orderOfBursts [], int orderOfJobs [], int orderCount, int noOfJobs, boolean idle, int stack [][], boolean isAlive [][], double stackArrival [], int widthCount [], int stackCount, int widest) 
	{
	...
	launchFrame () 
	{
		...
		for (count = 0; count < orderCount; count++) {
			...
			if (orderOfJobs [count] == ProcessConstants.MAX_JOB) {
				lblJob [count] 		= 	new JLabel ("Overhead");
			} else {
