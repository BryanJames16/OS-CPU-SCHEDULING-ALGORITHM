...
PRIO 
{
	...
	protected 
	PRIO (double grandData [][], int jobNumber [], int noOfJobs) 
	{
	...
	compute () 
	{
		...
		if (idle) {
			for (timer = grandData [0][0]; timer < totalBurst; ) { 
				...
				int highestJobIndex = ProcessConstants.MIN_INDEX;
				...
				processedJobs [highestJobIndex] = true;
				timer += grandData [highestJobIndex][1];
				grandData [counter][4] = timer;
				flagTime [counter2] = timer;
				orderOfJobs [orderCount] = highestJobIndex;
				
				System.out.println ("    \t|    " + String.format ("J %d", jobNumber [highestJobIndex]) + "   |");
			...
			for (timer = grandData [0][0]; timer < totalBurst; ) { 
				...
				System.out.println (jobNumber [highestJobIndex]);
